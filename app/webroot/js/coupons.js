$(function() {
     ...
     */
    $('.tabla-carrito').on('click', '.set-coupon', function(e) {
        myCoupon = $('#CouponCode').val();
        verifyCoupon(myCoupon);
    });
			...
			BJS.get('/coupon_batches/getCouponInfo/' + couponCode, {}, function(couponData) {
				...
				if(couponInfo) {
                    ...
                    BJS.JSON('/b_cart/shopping_carts/get/1', {}, function(shoppingCartData) {
                        ...
                        totalCarrito = parseInt($('#TotalCarrito').attr('rel'));
                        costoEnvio = parseInt(priceFormat($('#CostoDeEnvio').text()));
                        if(subTotal > 0 && totalCarrito > subTotal) {
                            if(!isNaN(costoEnvio)) {
                                subTotal += costoEnvio;
                            }
