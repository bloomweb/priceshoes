...
$(function() {
	...
	$('.resumen-favoritos').on('click', '.removeFavoriteItem', function(e) {
		...
		BJS.JSON($that.attr('href'), {}, function(response) {
			if (response.success) {
                ...
                var n = d.getMilliseconds();
                //$('.resumen-carrito').load('/pages/resumenCarrito?_t='+n);
                $('.resumen-favoritos').load('/pages/resumenFavoritos?_t='+n);
			} else {
				alert(response.message);
			}
		});
	...
	$('.tabla-carrito').on('click', '.removeFavoriteItem', function(e) {
		...
		BJS.JSON($that.attr('href'), {}, function(response) {
			if (response.success) {
                var d = new Date();
                var n = d.getMilliseconds();
				$('.tabla-carrito .content').load('/pages/tablaFavoritos?_t='+n, function() {
					$('.actualizando').hide();
				});
			} else {
				alert(response.message);
	...
	galeria = {
		...
		init : function() {
			...
			$(".galeria").on("click", ".zoom-in", function() {
				...
				if (newWidth <= that.MAX_WIDTH) {
					$(".imagen-container img").animate({
						"width" : newWidth,
						"height" : newWidth,
						left : that.last_left - (that.zoom_ratio / 2),
						top : that.last_top - (that.zoom_ratio / 2)
					}, that.DURATION);
				}
			});
		...
		reset : function() {
			that = this;
			$(".imagen-container img").animate({
				"width" : that.ORIGINAL_WIDTH,
				"height" : that.ORIGINAL_HEIGHT,
				left : 0,
				top : 0
			}, that.DURATION);
		},
		change : function(gallery) {
			if ( typeof gallery['Image']['0'] != "undefined") {
				...
				$(".imagen-container img").draggable({
