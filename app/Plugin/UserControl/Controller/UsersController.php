<?php
	...
	class UsersController extends UserControlAppController {
		...
		public function beforeFilter() {
			...
			$this->Auth->allow('admin_login', 'login', 'register', 'registerEmail', 'resetPassword', 'createUser', 'logout', 'admin_logout', 'profile', 'loginFacebook', 'logoutFacebook', 'facebook_connect');
		...
		private function getFacebook() {
			// Crear el objeto facebook
			$lib_path = APP . 'Plugin/UserControl/Lib/facebook/src/facebook.php';
			require_once($lib_path);
			$facebook_config = array(
				'appId'              => '626363840745969',
				'secret'             => '749f06eec4d3a3b1367bcad564c43981',
		...
		public function loginFacebook() {
			...
			$redirect_url = FULL_BASE_URL . '/user_control/users/facebook_connect';
		...
		public function facebook_connect() {
			...
			if($fb_user) {
				try {
					...
					// En caso que no registrarlo
					$user_id = $this->requestAction('/user_control/oauthfbs/checkFBID/' . $fb_user);

					if($user_id) {
						$this->internalLoginUser($user_id);
					} else {
						$uno = 1;
						...
						$user_data = array(
							'User' => array(
								...
								'birthday' => $birthday,
								'sex' => $user_profile['gender'] == 'male' ? 'M' : 'F',
								'is_active' => 1
							)
						);
						$this->User->create();
						if($this->User->save($user_data)) {
							...
							$result = $this->sendRegistrationEmail($user_data);
							if($result) {
							...
							$this->requestAction('/user_control/oauthfbs/createRelationship/' . $fb_user . '/' . $user_id);
							$this->internalLoginUser($user_id);
						} else {
							// Verificar si falló el registro porque ya existe el usuario
							if(
								isset($this->User->validationErrors['username'])
								&& isset($this->User->validationErrors['email'])
							) {
								// Asignar el fb_id al usuario con el correo
								$user = $this->User->findByEmail($user_profile['email']);
								$user_id = $user['User']['id'];
								$this->requestAction('/user_control/oauthfbs/createRelationship/' . $fb_user . '/' . $user_id);
								$this->internalLoginUser($user_id);
							}
		...
		/**
		 * Menejar el logout via facebook
		 */
